...
class qlykho{
    ...
    public function chitietsanpham($sql){
		...
		if($i>0)
		{
			...
			{
                $idSP = $row['idSanPham'];
                $laytenSP= $this->laycot("select tensanPham from sanpham sp join chitietyeucauxuatsp ct on sp.idSanPham = ct.idSanPham where ct.idSanPham = '$idSP' ");
                $laysoluongSP= $this->laycot("select soLuongXuat from chitietyeucauxuatsp where idSanPham='$idSP'");
